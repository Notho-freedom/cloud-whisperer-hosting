    ...
    <div className="flex h-full flex-col">
      ...
      <div className="px-3 py-4">
        <Button asChild className="w-full justify-start gap-2" size="sm">
          <Link to="/app/sites/new" onClick={onNavigate}>
      ...
      <div className="border-t border-sidebar-border p-3">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">Passez Pro</span>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            Domaines inclus, support prioritaire & plus.
          </p>
        </Link>
      </div>
    </div>
...
function AppHeader({ onOpenMobile }: { onOpenMobile: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur md:px-6">
      <Button variant="ghost" size="icon" className="lg:hidden" onClick={onOpenMobile} aria-label="Menu">
      ...
      <div className="relative hidden flex-1 max-w-md md:block">
        ...
        <Input
          placeholder="Rechercher domaine, site, facture…"
          className="h-9 pl-9 pr-14"
        />
        <kbd className="pointer-events-none absolute right-2 top-1/2 hidden -translate-y-1/2 select-none items-center gap-1 rounded border border-border bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground sm:inline-flex">
          ⌘K
        </kbd>
      </div>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={() => navigate({ to: "/app/settings/profile" })}>
